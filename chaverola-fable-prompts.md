...
Build the teacher's live activity page and wire in the teacher chat cards from Prompt 2. Before changing any existing behavior, skim DECISIONS.md — the chat-end, reveal, and removal rules govern several behaviors below, and the "Teacher activity setup" section governs the fields the live settings panel mirrors — plus AGENTS.md for the shared chat layout and working conventions. Record new product rules from this prompt in DECISIONS.md as you implement them.

### Live activity page — `/activity/host/:joinCode`

Meta title: `Chaverola | Teachers View`

One piece of context shapes this whole page: unlike Kahoot, **a teacher never projects or shares this screen**. If students could see the waiting lobby, they'd know exactly who they're about to be matched with — and the who-am-I-chatting-with mystery is the whole game. This page is the teacher's private control room; anything students need to know, the teacher relays out loud or writes up. Record this no-projection principle in DECISIONS.md.

**Layout.** On phones, the page is stacked minimizable sections, in order: student joining instructions, edit activity settings, pair your students, chats in progress, completed chats. The settings section starts **collapsed** — it's already filled in from setup; every other section starts **open** — they begin mostly empty and are where the action is. Collapsed sections earn their closed state with a useful subtext (specified per section below). Build all of them on **one shared collapsible-section component**; there's no accordion primitive in `client/src/components/ui/` yet, so add one (or hand-roll it if that's cleaner). On desktop, "Pair your students" becomes a fixed-width **sticky left rail** (~320px, game-lobby-sidebar energy) with the chats — in progress, then completed — owning the remaining width, so the teacher watches the lobby refill while monitoring chats: their two core mid-round jobs, side by side. Instructions and settings sit full-width above the split. The rail is deliberately not "balanced" against the chats column: it's a persistent queue (20 students at round start, 0 mid-round, refilling as chats end), and it never disappears at zero — students come back to it. Record the layout rule in DECISIONS.md.

**Header:** the count of students waiting to chat is the page's hero stat — a teacher glancing over from across the classroom must catch it instantly, and it should visibly react when the count changes, so the teacher always knows there are students to match up. How you achieve that (sticky or not, size, motion) is your call. A status label like "Activity in Progress" is not required — add a status only if it earns its place.

- **Student joining instructions (minimizable):** shows the activity pin prominently and tells the teacher how students get in: students go to www.chaverola.com, click Join an Activity, and enter the pin. You choose the wording that best helps a teacher relay this to a class (out loud or on the board — never a shared screen, per the no-projection principle). Also include a **copyable direct link**: styled link-text (e.g., "Copy the student join link") that on click copies `www.chaverola.com/activity/join/:joinCode` to the clipboard and briefly confirms it ("Copied!") — never print the raw URL in the UI.
- **Edit activity settings (minimizable, starts collapsed):** all setup fields/toggles from Prompt 7 editable live — character rows with their optional emoji slots, hosted-by, teacher email, scene, and the four settings toggles. Don't rebuild any of it: reuse the Prompt 7 building blocks in `client/src/components/Teacher/ActivitySetup/` (`CharacterRowsField`, `EmojiSlot`, `SettingsSection`, `NumberStepper`, `FieldFeedback`) and the caps/validation in `client/src/lib/activitySetup.ts`, so the panel presents the same flat layout the teacher learned on the setup form (that mirroring is why setup is a flat form — see DECISIONS.md → "Setup is one scrolling form, and Host the Activity is never disabled"). All setup rules hold live: 30-char names/hosted-by with quiet counters, 20-word scene, duplicate names blocked, steppers visible-but-disabled when their toggle is off. New live-edit rules (record each in DECISIONS.md):
  - A valid edit applies **everywhere, immediately** — roster chips, pairing UI, in-progress chat cards, and student-facing surfaces all re-label. The whole class shares one character set and one scene; that's what lets the teacher rename the characters and reset the scene between rounds. Make this legible in the UI itself: the section carries a short hint that changes reach everyone instantly, and that the natural moment to switch up characters and scene is **between rounds**, after a round of chats ends.
  - Propagation is **debounced 1 second**: the input updates on every keystroke, but the change spreads only after the teacher pauses typing for a second, so half-typed names never flash across chat cards.
  - An invalid in-between state (a name cleared to empty, a duplicate name, a malformed email) shows the setup form's inline error while the **last valid value stays in effect** — students never see a blank or duplicate label. Fixing the field applies it on the next debounce tick.
  - **One exception to "immediately": the auto-end minutes.** A changed duration applies to chats started after the change; running chats keep the clock they started with — a shortened setting must never instantly kill a live chat mid-sentence.
  - The remove control on rows 3–4 is **disabled with a short hint while any in-progress chat uses that character**, and returns once those chats end; future pairings stop offering a removed character immediately.
  - If the teacher's email was not set, the collapsed section shows a subtext nudging them to add it.
- **Pair your students (the desktop rail; a minimizable section on mobile whose collapsed subtext shows how many students are waiting):**
  - Waiting students render as chips in **join order — new joiners append at the bottom**, so the longest-waiting students sit at the top and the list never reshuffles under the teacher's cursor. Each chip shows a subtle wait time (e.g., "2m").
  - **Pairing is tap-to-select:** tapping a chip selects it; with two selected, a start-chat action activates. With 3 characters defined, a third student can be selected for a group of 3; with 4, up to four. Characters are assigned **randomly** — no assignment step; the teacher sees who got whom on the chat card. Record the tap-to-select + random-assignment rule in DECISIONS.md.
  - A **Pair everyone** button pairs all waiting students into 1:1 chats. Odd count: if the roster has 3+ characters, the last three students form a group of 3 so nobody sits out; with only 2 characters, one student stays waiting, clearly highlighted (e.g., "first in line next round"). Record in DECISIONS.md.
  - Per setting #3, show a visual warning the moment a selected combo would rematch students **whose previous chat was with each other** — the rematch memory is one round deep, not the whole activity. The warning never blocks anything; pairing anyway stays one tap. Record the previous-round-only rule in DECISIONS.md.
  - **Removing a student must be obvious:** each chip carries its own small remove control (an ✕ or a ⋯ menu — your call), kept separate from tap-to-select so the two gestures never collide. It opens a confirmation modal (teachers typically remove students who joined with a made-up or inappropriate name). A removed student lands back on the **name step** at `/activity/join/:joinCode`, signed out, seeing a message that the teacher removed them and they need to sign in again (already implemented on the student side — see DECISIONS.md → "Student sign-in lives in the tab, and removal sends you to the name step").
  - When setting #4 (auto-match) is on, the demo simulates it: waiting students who've passed the wait threshold pair up on their own; toggling it off stops it.
  - Empty states matter here: at zero waiting, the rail shows a calm "everyone's chatting" state (students come back to the queue when their chats end and they click back to the lobby); before the first pairing, the chats area invites the teacher to pair students, surfacing the Pair-everyone action.
- **Chats in progress (minimizable, title shows the number of chats in progress):** inside: a toggle to reveal real names to the students, a count of how many students are chatting, and an "End all chats" button (opens a confirmation modal before ending all chats). Each chat renders as the teacher chat card from Prompt 2 (last 5 lines, expand/minimize, and end chat with its confirmation modal), now with two additions:
  - **The chat's remaining time**, when auto-end is on — the same per-chat clock the students see (see "The auto-end clock" below).
  - **A per-participant remove control** (with a confirmation modal), because a student being inappropriate mid-chat shouldn't be unstoppable. Removal is a **quiet exit** — never announced to students, so classmates can't tell a removal from a dropped connection (record in DECISIONS.md): in a group chat, a neutral notice appears ("{character} left the chat", composed via `characterLabel`) and the chat continues — mirroring the group disconnect-drop rule, with lines and colors persisting via `participants`; in a 1:1, the peer's chat ends with the existing 🎓 teacher copy. The removed student lands on the name step signed out, exactly like a lobby removal.
...
- **Completed chats (minimizable, title shows the number completed):** inside, a count plus the muted completed-chat cards with expand/minimize.

Visually, the page is the setup page's sibling: the same warm brand pass — accent icon chips per section (reuse `FormSection`'s accent idiom where it fits), calm card surfaces, a soft header glow — and never the purple student world, which stays a student-side identity (see DECISIONS.md → "Setup sections each carry one brand accent; settings stays the quiet one").

### The auto-end clock (both sides)

Today, students have no idea how much time is left in a chat — the ⏰ "Time's up!" ended copy is the first they hear of it. Fix that here (record the rules in DECISIONS.md):

- The clock is **per chat**: it starts when the chat starts, counting down the activity's auto-end minutes. When the teacher pairs everyone at once, chats naturally end together — and "End all chats" is the round-closer when the teacher wants everyone done now.
- Model it in the chat-engine contract: `ChatRoomState` gains an auto-end countdown (e.g., `autoEndSecondsLeft: number | null` — null when auto-end is off), ticked by `useChatDemo`, ending the chat with reason `"timer"` at zero.
- **Student view — subtle clock, loud finale:** a quiet m:ss countdown in the chat header (reuse the m:ss formatting the reconnect banner uses); in the final 60 seconds it shifts color/pulses so students wrap up their scene. On narrow widths, compress the "End chat" pill (e.g., icon + "End", with a proper aria-label) so the clock fits without crowding the header.
- **Teacher chat cards** show the same per-chat remaining time.
- Give the demo a **fast-forward trigger** (same spirit as the reconnect window's "skip the wait") so the finale state and the timer expiry are testable without sitting through 7 minutes.

### Mock/demo behavior

- The host page's demo data hosts the **same activity as the student side**: join code `1234`, hosted by Ms. Cohen, the Rome scenario and its four characters from `client/src/mockData/activityDemo.ts` — including the deliberately emoji-less Marc Antony, so teacher chat cards and pairing UI exercise name-only labels. The pin shown on screen is therefore the one that actually works on `/activity/join` in another tab. There is no cross-tab sync — both sides stay independently mocked; they just agree on the story.
- Arriving from Prompt 7's create flow, the page shows the activity the teacher just hosted instead: read it with `readHostedActivity(joinCode)` from `client/src/lib/activitySetup.ts` — the setup form stashes a `HostedActivity` (types in `client/src/types/activity.ts`) under the sessionStorage key `chaverola.hostedActivity`, with a code from `mockGenerateJoinCode` in `client/src/mockData/activityDemo.ts` (4-digit, never `1234`). A refresh keeps the hosted activity — the stash is per-tab, same spirit as the setup draft and the student session.
- Rome is the fallback seed for direct visits to `/activity/host/1234`: wrap `demoActivity` as a `HostedActivity` with `DEFAULT_ACTIVITY_SETTINGS` and **no teacher email**, so direct visits also exercise the collapsed settings section's add-your-email nudge. Any other code with no matching stash (say, an old link opened in a new tab) **redirects to `/activity/host/1234`**, so the pin on screen, the URL, and the copyable student link always agree — and the pin always works on `/activity/join`. Record the redirect rule in DECISIONS.md.
- Drive the in-progress mock chats through the chat-engine contract (`ChatRoomState`/`ChatRoomActions` in `client/src/types/chat.ts`) like every other chat surface — `useChatDemo` in `client/src/components/chat/` is the mock engine — and build any dev-only triggers from the demo kit in `client/src/components/demo/`.
- Seed ~6 fake students in the lobby, 2 chats in progress with live-updating mock messages, and 2 completed chats.
- Simulate students joining over time, so the waiting count changes and new joiners visibly append to the bottom of the queue; when auto-match is on, simulate waiting students pairing up on their own.

### Acceptance checklist

- [ ] Waiting count is unmissable and visibly reacts to changes; no mandated status label
- [ ] Mobile: stacked collapsible sections with settings collapsed by default and all others open; desktop: sticky pairing rail beside the chats area, instructions/settings full-width above
- [ ] All sections share one collapsible component; collapsed states show their subtexts (waiting count, email nudge) and titles show their counts
- [ ] Joining instructions show the pin prominently; the copy-link is styled text that copies the direct URL with a "Copied!" confirmation — no raw URL printed
- [ ] Direct visits host the Rome demo activity with pin `1234`, matching the student side; arriving from the create flow shows the just-hosted activity from `chaverola.hostedActivity`; an unknown code redirects to `/activity/host/1234`
- [ ] Settings panel reuses the ActivitySetup field components and validation; the setup caps, counters, and duplicate rules hold live; the applies-instantly / between-rounds hint is present
- [ ] Live edits propagate everywhere on a 1-second debounce; invalid edits show inline errors while the last valid value stays in effect; remove is disabled for characters in live chats; auto-end minute changes affect only chats started afterward
- [ ] Chips append in join order with wait times; tap-to-select pairing with random character assignment; groups of 3/4 per character count; Pair-everyone handles odd counts (trio when 3+ characters, highlighted leftover otherwise)
- [ ] Rematch warning fires on selection, remembers the previous round only, and never blocks pairing
- [ ] Every chip has an obvious remove control with a confirmation modal; per-participant remove works mid-chat with quiet-exit semantics (group continues with a neutral notice; 1:1 peer gets the 🎓 teacher copy)
...
- [ ] Auto-end countdown shows on teacher cards and in the student chat header with the final-minute emphasis; the End button compresses on narrow widths; a demo fast-forward makes the expiry testable
- [ ] New rules recorded in DECISIONS.md (no-projection, layout, tap-to-pair + random assignment, odd-count trio, previous-round rematch memory, quiet-exit removal, per-chat clock + finale, auto-end edit exception, redirect rule)
- [ ] Mobile friendly throughout
