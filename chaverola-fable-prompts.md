...
Build the teacher's activity setup page.

### Setup page — `/activity/create`
Meta title: `Chaverola | Teachers View`

Fields:
- **Characters (required, 2–4):** two character inputs by default. Placeholder for the first: "Caesar's ghost 👻". Placeholder for the second: "Brutus 🔪". Each character input has an emoji picker button so the teacher can add emojis to character names (emojis only — no stickers or GIFs; reuse the emoji picker from the student chatbox). An "add character" action allows up to 4 total. Show a note: starting with two characters (1:1 chats) is recommended; a 3rd character is only used when a group of 3 students is paired, and a 4th only for a group of 4. At least two characters are required to start.
...
- [ ] Emoji picker on each character input works
...
Build the teacher's live activity page and wire in the teacher chat cards from Prompt 2.
...
- **Edit activity settings (accordion, minimizable):** all setup fields/toggles from Prompt 7 editable live (including character names with their emoji pickers). If the teacher's email was not set, the closed accordion shows a subtext nudging them to add it.
- **Pair your students (accordion, minimizable):** closed accordion subtext shows how many students are waiting to start.
  - Clicking a student opens a confirmation modal to remove them (teachers typically remove students who joined with a made-up or inappropriate name). A removed student goes back to the lobby at `/activity/join/:joinCode` and sees a message that the teacher removed them and they need to sign in again.
